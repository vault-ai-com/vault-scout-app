-- Migration: Create get_player_football_stats RPC
-- Fixes phantom RPC — this function was called by scout-analyze-player and
-- scout-personality-analysis but never existed, causing silent null returns
-- and downstream hallucinations.
--
-- Aggregates football_player_stats joined with football_fixtures to return
-- match count, aggregated stats, and recent matches for a given player name.
...
BEGIN
  ...
  WHERE lower(ps.player_name) = lower(p_player_name)
...
-- Grant access for edge functions (service_role + anon for webapp)
