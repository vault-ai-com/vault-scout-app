...
interface CoachPersonalityPanelProps {
  data: CoachPersonalityResponse | null;
  loading: boolean;
  error: string | null;
  onAnalyze: () => void;
}
...
const DIMENSION_LABELS: Record<string, string> = {
  ...
  team_orientation: "Lagorientering",
  tactical_innovation: "Taktisk innovation",
  ambition_level: "Ambitionsnivå",
  career_motivation: "Karriärmotivation",
  ego: "Ego",
  resilience: "Resiliens",
  learning_orientation: "Lärandeorientering",
  x_factor: "X-faktor",
};
...
function ScoreBar({ score, label, evidence }: { score: number; label: string; evidence: string }) {
  ...
  const color = clamped >= 7 ? "bg-success" : clamped >= 4 ? "bg-warning" : "bg-destructive";
...
function getTopDimensions(profile: CoachPersonalityResponse["profile"]) {
  return DIMENSION_KEYS
    .map((key) => {
      const dim = profile[key] as { score: number; evidence: string } | undefined;
      if (!dim) return null;
...
export function CoachPersonalityPanel({ data, loading, error, onAnalyze }: CoachPersonalityPanelProps) {
    ...
    <div className="card-editorial card-accent-left p-6 md:p-8 space-y-4">
      ...
      {loading && (
        <div className="flex items-center gap-2 py-4" role="status" aria-live="polite">
          ...
          <span className="text-sm text-muted-foreground">Analyserar tränarpersonlighet...</span>
        </div>
      )}
      ...
      {profile && !loading && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          {/* Archetype badge + data source */}
          <div className="flex items-center gap-2">
            {profile.composite_archetype && (
              <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-semibold border ${COACH_ARCHETYPE_COLORS[profile.composite_archetype] ?? "bg-zinc-500/10 text-zinc-400 border-zinc-500/20"}`}>
                <Zap className="w-3.5 h-3.5" />
                {COACH_ARCHETYPE_LABELS[profile.composite_archetype] ?? profile.composite_archetype}
              </span>
            )}
            {profile.data_source_quality && (
              <span className="inline-flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-medium text-muted-foreground border border-border/30 bg-card/40">
                ...
                {DATA_SOURCE_LABELS[profile.data_source_quality as DataSourceQuality] ?? profile.data_source_quality}
              </span>
            )}
          </div>
          ...
          <div className="space-y-3">
            {DIMENSION_KEYS.map((key) => {
              ...
              if (!dim) return null;
              return (
                <ScoreBar key={key} score={dim.score} label={DIMENSION_LABELS[key] ?? key} evidence={dim.evidence} />
              );
            })}

            {/* Contradiction score (0-1 scale) */}
            {profile.contradiction_score && (() => {
              const cs = profile.contradiction_score;
          ...
          {data?.recommendation && (
            <div className="rounded-lg p-3 bg-accent/5 border border-accent/10">
              <span className="text-[10px] font-semibold text-accent uppercase tracking-wider">Rekommendation</span>
              <p className="text-sm text-foreground/90 mt-1">{data.recommendation}</p>
            </div>
          )}
